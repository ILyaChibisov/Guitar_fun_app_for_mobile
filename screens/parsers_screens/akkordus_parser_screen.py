# screens/parsers_screens/akkordus_parser_screen.py
"""
Экран управления парсером Akkordus.Ru - парсит по группам
"""
...
class AkkordusRecentSongCard(MDCard):
...
class AkkordusParserScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.name = 'akkordus_parser'
        ...
        self.load_icon()
        self.init_ui()
        logger.info('Экран Akkordus парсера создан')
    ...
    def init_ui(self):
        ...
        title_label = MDLabel(text="AKKORDUS ПАРСЕР", font_size=sp(22), halign="center", bold=True,
                              theme_text_color="Custom", text_color=[0.7, 0.4, 0.8, 1])
        subtitle_label = MDLabel(text="загрузка аккордов с akkordus.ru", font_size=sp(11), halign="center",
        ...
        # Кнопки управления
        buttons_card = MDCard(orientation='vertical', size_hint=(1, None), height=dp(65), padding=[dp(12), dp(8), dp(12), dp(8)],
        ...
        self.start_btn = MDRaisedButton(text="ЗАПУСТИТЬ", size_hint_x=0.7, md_bg_color=[0.2, 0.6, 0.2, 1], font_size=sp(14))
        ...
        self.total_card = AkkordusStatCard("ВСЕГО", 0, [0.4, 0.7, 0.9])
        self.new_card = AkkordusStatCard("НОВЫЕ", 0, [0.3, 0.8, 0.3])
        self.dup_card = AkkordusStatCard("ПОВТОР", 0, [0.9, 0.7, 0.2])
        self.err_card = AkkordusStatCard("ОШИБКИ", 0, [0.9, 0.4, 0.4])
            ...
            if result and result.get('success'):
                ...
                if last_song and last_song.get('filename'):
                    if self.last_song != last_song.get('filename'):
                        ...
                        song_card = AkkordusRecentSongCard(song_data=last_song, icon_data=self.song_icon_data)
