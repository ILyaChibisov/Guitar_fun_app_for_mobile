...
"""
Экран управления парсером MyTabs - переведён на BaseParserScreen
"""
...
class MyTabsLetterSelector(ButtonBehavior, BoxLayout):
    ...
    def _create_popup(self):
        ...
        header_title = Label(
            text="ВЫБЕРИТЕ БУКВУ (MYTABS)", font_size=sp(16),
...
class MyTabsParserScreen(BaseParserScreen):
    def __init__(self, **kwargs):
        ...
        self.load_icon()
        self.init_ui()
        logger.info('Экран MyTabs парсера создан (BaseParserScreen)')
    ...
    def init_ui(self):
        ...
        title_label = MDLabel(
            text="MYTABS ПАРСЕР", font_size=sp(22), halign="center",
            bold=True, theme_text_color="Custom", text_color=[0.7, 0.5, 0.9, 1]
        )
        subtitle_label = MDLabel(
            text="загрузка аккордов с mytabs.ru", font_size=sp(11),
        ...
        self.start_letter_selector = MyTabsLetterSelector(title="ОТ", on_select=self.on_start_letter_selected)
        self.end_letter_selector = MyTabsLetterSelector(title="ДО", on_select=self.on_end_letter_selected)
