...
"""
Экран управления парсером 5Lad.Net - парсит по группам
"""
...
class FiveLadParserScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        self.load_icon()
        self.init_ui()
        logger.info('Экран 5Lad парсера создан')
    ...
    def init_ui(self):
        ...
        title_label = MDLabel(text="5LAD ПАРСЕР", font_size=sp(22), halign="center", bold=True,
                              theme_text_color="Custom", text_color=[0.8, 0.6, 0.2, 1])
        subtitle_label = MDLabel(text="загрузка аккордов с 5lad.net", font_size=sp(11), halign="center",
    ...
    def _fetch_status(self):
        try:
            result = api.get_fivelad_parser_status_sync()
    ...
    def start_parser(self, *args):
        try:
            result = api.start_fivelad_parser_sync(0, 999)
            if result and result.get('success'):
    ...
    def stop_parser(self, *args):
        try:
            result = api.stop_fivelad_parser_sync()
            if result and result.get('success'):
