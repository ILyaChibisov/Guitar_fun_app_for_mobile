# screens/parsers_screens/fivelad_parser_screen.py
"""
Экран управления парсером 5Lad.Net - переведён на BaseParserScreen
"""
...
logger = screen_logger('FiveLadParserScreen')
...
class FiveLadRecentSongCard(MDCard):
...
class FiveLadParserScreen(BaseParserScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.name = 'fivelad_parser'
        ...
        self.load_icon()
        self.init_ui()
        logger.info('Экран 5Lad парсера создан (BaseParserScreen)')
    ...
    def init_ui(self):
        ...
        title_label = MDLabel(
            text="5LAD ПАРСЕР", font_size=sp(22), halign="center",
            bold=True, theme_text_color="Custom", text_color=[0.8, 0.6, 0.2, 1]
        )
        subtitle_label = MDLabel(
            text="загрузка аккордов с 5lad.net", font_size=sp(11),
        ...
        self.total_card = FiveLadStatCard("ВСЕГО", 0, [0.4, 0.7, 0.9])
        self.new_card = FiveLadStatCard("НОВЫЕ", 0, [0.3, 0.8, 0.3])
        self.dup_card = FiveLadStatCard("ПОВТОР", 0, [0.9, 0.7, 0.2])
        self.err_card = FiveLadStatCard("ОШИБКИ", 0, [0.9, 0.4, 0.4])
    ...
    def _update_last_song_display(self, last_song):
        ...
        if self._is_real_song(filename, status):
            if self.last_song != filename:
                ...
                song_card = FiveLadRecentSongCard(song_data=last_song, icon_data=self.song_icon_data)
    ...
    def _fetch_status(self):
        try:
            result = parser_client.get_fivelad_parser_status_sync()
    ...
    def start_parser(self, *args):
        try:
            result = parser_client.start_fivelad_parser_sync(0, 999)
            if result and result.get('success'):
                notify.success("Парсер 5Lad.Net запущен")
                self.last_song = None
                self.last_song_container.height = dp(0)
                self.last_song_container.clear_widgets()
                self._fetch_status()
    ...
    def stop_parser(self, *args):
        try:
            result = parser_client.stop_fivelad_parser_sync()
            if result and result.get('success'):
                notify.info("Парсер 5Lad.Net остановлен")
                self._fetch_status()
        except Exception as e:
            logger.error(f"Ошибка: {e}")
            notify.error(f"Ошибка: {e}")
