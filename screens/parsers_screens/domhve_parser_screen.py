# screens/parsers_screens/domhve_parser_screen.py
"""
Экран управления парсером Domhve.com - все песни подряд
"""
...
logger = screen_logger('DomhveParserScreen')
...
class DomhveRecentSongCard(MDCard):
...
class DomhveParserScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.name = 'domhve_parser'
        ...
        self.load_icon()
        self.init_ui()
        logger.info('Экран Domhve парсера создан')
    ...
    def init_ui(self):
        ...
        title_label = MDLabel(text="DOMHVE ПАРСЕР", font_size=sp(22), halign="center", bold=True,
                              theme_text_color="Custom", text_color=[0.3, 0.8, 0.7, 1])
        subtitle_label = MDLabel(text="христианские песни с domhve.com", font_size=sp(11), halign="center",
        ...
        self.total_card = DomhveStatCard("ВСЕГО", 0, [0.4, 0.7, 0.9])
        self.new_card = DomhveStatCard("НОВЫЕ", 0, [0.3, 0.8, 0.3])
        self.dup_card = DomhveStatCard("ПОВТОР", 0, [0.9, 0.7, 0.2])
        self.err_card = DomhveStatCard("ОШИБКИ", 0, [0.9, 0.4, 0.4])
    ...
    def _fetch_status(self):
        try:
            result = api.get_domhve_parser_status_sync()
    ...
    def start_parser(self, *args):
        try:
            result = api.start_domhve_parser_sync(0, 999)
            if result and result.get('success'):
                notify.success("Парсер Domhve.com запущен")
                self.last_song = None
                self.last_song_container.height = dp(0)
                self.last_song_container.clear_widgets()
                self._fetch_status()
    ...
    def stop_parser(self, *args):
        try:
            result = api.stop_domhve_parser_sync()
            if result and result.get('success'):
                notify.info("Парсер Domhve.com остановлен")
                self._fetch_status()
        except Exception as e:
            logger.error(f"Ошибка: {e}")
            notify.error(f"Ошибка: {e}")
