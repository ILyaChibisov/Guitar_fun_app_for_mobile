...
"""
Экран управления парсером Domhve.com - переведён на BaseParserScreen
"""
...
class DomhveParserScreen(BaseParserScreen):
    def __init__(self, **kwargs):
        ...
        self.load_icon()
        self.init_ui()
        logger.info('Экран Domhve парсера создан (BaseParserScreen)')
    ...
    def init_ui(self):
        ...
        title_label = MDLabel(
            text="DOMHVE ПАРСЕР", font_size=sp(22), halign="center",
            bold=True, theme_text_color="Custom", text_color=[0.3, 0.8, 0.7, 1]
        )
        subtitle_label = MDLabel(
            text="христианские песни с domhve.com", font_size=sp(11),
    ...
    def _fetch_status(self):
        try:
            result = parser_client.get_domhve_parser_status_sync()
    ...
    def start_parser(self, *args):
        try:
            result = parser_client.start_domhve_parser_sync(0, 999)
            if result and result.get('success'):
    ...
    def stop_parser(self, *args):
        try:
            result = parser_client.stop_domhve_parser_sync()
            if result and result.get('success'):
